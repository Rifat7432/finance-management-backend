...
const createPartnerRequestToDB = async (inviterId: string, partnerData: { name: string; email: string; relation: string }) => {
     ...
     if (existingUser) {
          ...
          // Link the partner user with the inviter
          inviter.partnerId = newPartner._id;
          await inviter.save();
...
const updatePartnerRequestToDB = async (id: string, payload: Partial<IPartnerRequest>): Promise<IPartnerRequest | null> => {
     const updated = await PartnerRequest.findByIdAndUpdate(id, payload, { new: true });
     if (!updated) {
          throw new AppError(StatusCodes.BAD_REQUEST, 'Failed to update partner request');
     }
     return updated;
};
